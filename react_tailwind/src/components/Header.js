...
const Header = ({title = "Tailwind CSS"}) => {
    const [showNav, setShowNav] = useState(false);

    return (
        <nav className="flex items-center justify-between flex-wrap bg-white p-6 border-b border-gray-200 dark:border-gray-300">
            ...
            <label className="block lg:hidden cursor-pointer flex items-center px-3 py-2 border rounded text-gray-700 border-gray-400 hover:text-gray-500 hover:border-gray" for="menu-toggle">
            ...
            <div className={`overflow-hidden lg:overflow-visible w-full h-0 block flex-grow lg:flex lg:items-center lg:w-auto transition-all ease-out duration-500 lg:transition-none lg:h-auto ${showNav ? 'h-32': ''}`} id="menu">
                <div className="flex flex-col lg:flex-row text-sm lg:flex-grow duration-300 ease-out sm:transition-none">
                    ...
                    <a href="#responsive-header" className="block mt-4 lg:inline-block lg:mt-0 text-gray-500 hover:text-gray-700">
                        Blog
                    </a>
                </div>
            </div>
